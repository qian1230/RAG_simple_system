    ...
    def encode(self, texts: Union[str, List[str]]):
        ...
        if self.base_url:
            ...
            try:
                ...
                if not items:
                    raise RuntimeError(f"REST 返回数据为空: {data}")
                vecs = []
                for item in items:
                    embedding = item.get("embedding")
                    if not embedding:
                        raise RuntimeError(f"REST 返回缺少embedding字段: {item}")
                    vecs.append(np.array(embedding))
                if single:
                    return vecs[0]
                return vecs
            except Exception as e:
                print(f"[ERROR] REST模式编码失败: {e}")
                raise
        ...
        try:
            ...
            print(f"[DEBUG] DashScope返回类型: {type(rsp)}")
            
            # 处理不同的返回格式
            embeddings_obj = None
            if isinstance(rsp, dict):
                print(f"[DEBUG] DashScope返回字典结构: {list(rsp.keys())}")
                output = rsp.get("output", {})
                print(f"[DEBUG] Output结构: {list(output.keys()) if isinstance(output, dict) else output}")
                embeddings_obj = output.get("embeddings")
            else:
                print(f"[DEBUG] DashScope返回对象属性: {[attr for attr in dir(rsp) if not attr.startswith('_')]}")
                output = getattr(rsp, "output", None)
                if output:
                    print(f"[DEBUG] Output对象属性: {[attr for attr in dir(output) if not attr.startswith('_')]}")
                    embeddings_obj = getattr(output, "embeddings", None)
            
            print(f"[DEBUG] Embeddings对象: {embeddings_obj}")
            
            if not embeddings_obj:
                raise RuntimeError(f"DashScope 返回为空或格式不匹配: {rsp}")
            
            vecs = []
            for i, item in enumerate(embeddings_obj):
                print(f"[DEBUG] 第{i}个嵌入项: {type(item)} - {list(item.keys()) if isinstance(item, dict) else item}")
                embedding = item.get("embedding") or item.get("vector")
                if not embedding:
                    raise RuntimeError(f"嵌入项缺少embedding/vector字段: {item}")
                try:
                    vec = np.array(embedding)
                    vecs.append(vec)
                except Exception as e:
                    raise RuntimeError(f"向量转换失败: {e} - 嵌入数据: {embedding}")
            
            if single:
                return vecs[0]
            return vecs
        except Exception as e:
            print(f"[ERROR] SDK模式编码失败: {e}")
            raise
