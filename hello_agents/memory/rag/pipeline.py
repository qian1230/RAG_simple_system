            ...
            for v in part_vecs:
                try:
                    ...
                    if isinstance(v, (list, tuple)):
                        ...
                        # 标量或其他类型，使用零向量
                        print(f"[WARNING] 向量类型异常: {type(v)}, 使用零向量")
                    ...
                    for v in small_vecs:
                        ...
                        try:
                            ...
                            if isinstance(v, (list, tuple)):
                                ...
                                if len(v_norm) != dimension:
                                    print(f"[WARNING] 小批次向量维度异常: 期望{dimension}, 实际{len(v_norm)}")
                                    if len(v_norm) < dimension:
                                        v_norm.extend([0.0] * (dimension - len(v_norm)))
                                    else:
                                        v_norm = v_norm[:dimension]
                                ...
                                # 标量或其他类型，使用零向量
                                print(f"[WARNING] 小批次向量类型异常: {type(v)}, 使用零向量")
                                vecs.append([0.0] * dimension)
