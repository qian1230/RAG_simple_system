    ...
    def _initialize_client(self):
        ...
        try:
            ...
            if self.url and not self.url.startswith(('http://', 'https://')):
                # 如果URL不是以http或https开头，可能是集合名称被错误地当作URL传递了
                logger.error(f"[Qdrant] URL格式错误: {self.url}")
                logger.info("[Qdrant] 从环境变量重新获取正确的Qdrant URL")
                
                # 从环境变量获取正确的URL
                correct_url = os.getenv("QDRANT_URL")
                correct_api_key = os.getenv("QDRANT_API_KEY")
                
                if correct_url and correct_api_key:
                    logger.info(f"[Qdrant] 使用环境变量中的URL: {correct_url}")
                    self.url = correct_url
                    self.api_key = correct_api_key
                else:
                    logger.error("[Qdrant] 环境变量中未配置QDRANT_URL或QDRANT_API_KEY")
                    raise ValueError("Qdrant URL格式错误且环境变量未配置")
            
            # 根据配置创建客户端连接
            if self.url and self.api_key:
                # 使用云服务API
                logger.info(f"[Qdrant] 尝试连接到Qdrant云服务: {self.url}")
                ...
                try:
                    ...
                    logger.info(f"[Qdrant] 解析Qdrant地址: {hostname}:{port}")
                    
                    # 测试DNS解析
                    addrinfo = socket.getaddrinfo(hostname, port, socket.AF_UNSPEC, socket.SOCK_STREAM)
                    logger.info(f"[Qdrant] DNS解析成功: {addrinfo[0][4]}")
                    
                except socket.gaierror as e:
                    logger.error(f"[Qdrant] DNS解析失败: {e}")
                    logger.info("[Qdrant] 尝试使用备用IP地址...")
                    
                    # 尝试使用硬编码的IP地址（基于之前的DNS解析结果）
                    # 注意：这是临时解决方案，实际部署时应该使用正确的DNS配置
                    parsed_url = urlparse(self.url)
                    ...
                    # 硬编码的IP地址（基于之前的DNS解析结果）
                    backup_ip = "34.248.146.137"  # 假设这是解析出的IP地址
                    backup_url = f"{scheme}://{backup_ip}:{port}{path}"
                    if query:
                        backup_url += f"?{query}"
                    
                    logger.info(f"[Qdrant] 使用备用URL: {backup_url}")
                    self.url = backup_url
