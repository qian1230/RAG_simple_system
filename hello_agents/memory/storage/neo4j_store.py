...
class Neo4jGraphStore:
    ...
    def _initialize_driver(self, **config):
        """初始化Neo4j驱动"""
        try:
            # 为 Neo4j Aura 云服务创建 SSL 上下文
            if "neo4j.io" in self.uri or "aura" in self.uri.lower():
                logger.info("🔒 为 Neo4j Aura 配置 SSL 上下文...")
                # 创建自定义 SSL 上下文（强制加密+禁用证书验证）
                ssl_context = ssl.SSLContext(ssl.PROTOCOL_TLS_CLIENT)
                ssl_context.check_hostname = False
                ssl_context.verify_mode = ssl.CERT_NONE
                # 强制启用加密（匹配 Neo4j Aura 的加密要求）
                ssl_context.set_ciphers('DEFAULT')
                
                # 驱动初始化：bolt:// + ssl_context
                self.driver = GraphDatabase.driver(
                    self.uri,
                    auth=(self.username, self.password),
                    ssl_context=ssl_context,
                    encrypted=True,  # 强制加密，匹配 Aura 的要求
                    **config
                )
            else:
                # 本地 Neo4j 服务使用默认配置
            ...
            if "localhost" in self.uri:
                ...
                logger.info("💡 注意：Neo4j Aura 需要使用 bolt:// 协议 + SSL 上下文")
            raise
        except Exception as e:
            logger.error(f"❌ Neo4j连接失败: {e}")
            logger.info("💡 尝试使用 SSL 上下文配置...")
            raise
