...
class SemanticMemory(BaseMemory):
    ...
    def _init_databases(self):
        ...
        try:
            ...
            # 初始化Neo4j图数据库（如果配置了且连接成功）
            try:
                from ..storage.neo4j_store import Neo4jGraphStore
                neo4j_config = db_config.get_neo4j_config()
                if neo4j_config and all(neo4j_config.get(key) for key in ['uri', 'username', 'password']):
                    ...
                    self.graph_store = None
                    logger.warning("⚠️ Neo4j配置不完整，图搜索功能将不可用")
            except Exception as neo4j_error:
                self.graph_store = None
                logger.warning(f"⚠️ Neo4j初始化失败，图搜索功能将不可用: {neo4j_error}")
                logger.warning("💡 提示：Neo4j连接失败不会影响核心功能，程序将继续运行")
            ...
            logger.info(f"🏥 数据库健康状态: Qdrant={'✅' if vector_health else '❌'}, Neo4j={'✅' if self.graph_store else '❌'}")
    ...
    def _store_linguistic_analysis(self, doc, text: str):
        """存储spaCy词法分析结果到Neo4j"""
        if not self.graph_store:
            logger.debug("⚠️ 图数据库不可用，跳过词法分析存储")
            return
